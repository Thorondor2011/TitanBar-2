...
-- convert most values to string.format() then save
	if Locale == "de" then Turbine.PluginData.Save( Turbine.DataScope.Character, "TitanBar2SettingsDE", settings ); end
	if Locale == "en" then Turbine.PluginData.Save( Turbine.DataScope.Character, "TitanBar2SettingsEN", settings ); end
	if Locale == "fr" then Turbine.PluginData.Save( Turbine.DataScope.Character, "TitanBar2SettingsFR", settings ); end
end
...
--[[
function AddMenuItem(Ctr)
	local opt_line = Turbine.UI.MenuItem("---------------------------------------------", false);
	local opt_empty = Turbine.UI.MenuItem("", false);
	
	Ctr.MenuOpt = Turbine.UI.MenuItem(Ctr.NAME);
	Ctr.MenuOpt:SetChecked( Ctr.ShowMe );
	Ctr.MenuOpt = function( sender, args ) Ctr:ShowHide(); TitanBarMenu:ShowMenuAt(mouseXPos, mouseYPos); end
	TitanBarMenu.items:Add(Ctr.MenuOpt);
end]]
...
function GetCurrency(str)
	...
	for k,v in pairs(PlayerCurrency) do
		if k == str then
			CurQuantity = PlayerCurrency[k]:GetQuantity();
			break
		end
	end
	--write(CurQuantity);
...
function UnloadAllControls()
	-- unload wallet controls
	for k,v in pairs(TitanBar.WalletItemCtr) do
		if TitanBar.WalletItemCtr[k].ShowMe then
			TitanBar.WalletItemCtr[k].Where = 3;
			TitanBar.WalletItemCtr[k]:ShowHide();
		end
	end
	-- unload wallet
	if TitanBar.WalletCtr.ShowMe then
		TitanBar.WalletCtr:ShowHide();
		TitanBar.WalletCtr.MenuOpt:SetChecked( false );
	end
	
	TitanBar.Window.MouseLeave();
end

function BGColor( cmd, value, ctrl )
	if cmd == "reset" then
		if Locale == "en" then tA, tR, tG, tB = 0.3, 0.3, 0.3, 0.3;
		else tA, tR, tG, tB = tonumber("0,3"), tonumber("0,3"), tonumber("0,3"), tonumber("0,3"); end
	elseif cmd == "match" then
		tA, tR, tG, tB = TitanBar.bcAlpha, TitanBar.bcRed, TitanBar.bcGreen, TitanBar.bcBlue;
	elseif cmd == "apply" then
		tA, tR, tG, tB = ctrl.bcAlpha, ctrl.bcRed, ctrl.bcGreen, ctrl.bcBlue;
	end
	
	if value == "ctr" then
		ctrl.bcAlpha, ctrl.bcRed, ctrl.bcGreen, ctrl.bcBlue = tA, tR, tG, tB;
		settings[ctrl.NAME].A, settings[ctrl.NAME].R, settings[ctrl.NAME].G, settings[ctrl.NAME].B = tA, tR, tG, tB;
		if ctrl.ShowMe then ctrl.Ctr:SetBackColor( Turbine.UI.Color( tA, tR, tG, tB ) ); end
	elseif value == "all" then
		BGColor( cmd, "ctr", ctrl );
		BGColor( cmd, "TitanBar" );
	elseif value == "this" then
		ctrl.Ctr:SetBackColor( Turbine.UI.Color( tA, tR, tG, tB ) );
	elseif value == "TitanBar" then
		TitanBar.bcAlpha, TitanBar.bcRed, TitanBar.bcGreen, TitanBar.bcBlue = tA, tR, tG, tB;
		settings.TitanBar.A, settings.TitanBar.R, settings.TitanBar.G, settings.TitanBar.B = tA, tR, tG, tB;
		TitanBar.Window:SetBackColor( Turbine.UI.Color( tA, tR, tG, tB ) );
	end
